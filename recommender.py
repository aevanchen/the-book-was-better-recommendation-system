...
#     mod_bbt = np.where(book_by_trope==0, -x*c, book_by_trope*y*c)
    ...
    inital_query_vec = input_data[[input_name2id[title]]]
    query_vec = inital_query_vec
    ...
    if relevance_feedback:
        ...
        beta = 0.75
        gamma = 0.15
        top_k=5# choose top 2 as relevant query
...
#     print(key_word_scores)
    ...
    if keyword is not None and is_not_blank(keyword):
        print(keyword)
        scores= 1*key_word_scores+0*no_key_word_score
    else:
        scores=no_key_word_score
        

   
    ranked_indices,ranked_scores=findknn(np.reshape(scores,newshape=(1,-1)),100)
    ...
    for i in range(len(ranked_indices[0])):
         dot=np.multiply(output_data[[ranked_indices[0][i]]], query_vec[0])
